...
const DEFAULT_CONFIG = {
  templateEngine: null,
  templateDest: './components/5h3ll-ui',
  scriptDest: './static/js/5h3ll-ui'
};
...
async function getAvailableDesignSkins() {
  try {
    const cliScriptRunningDir = path.dirname(fileURLToPath(import.meta.url));
    const designMdDir = path.resolve(cliScriptRunningDir, 'assets', 'design-md');
    ...
    console.error('Error reading DESIGN.md skins from CLI assets:', error);
    return [];
  }
}

function resolveDesignSkin(name) {
  const normalized = String(name || '').trim().toLowerCase();
  return DESIGN_ALIASES[normalized] || normalized;
}

async function addDesignSkin(skinArg) {
  const skin = resolveDesignSkin(skinArg);
  console.log(`\nInstalling DESIGN.md skin: ${skin}...`);

  const cliScriptRunningDir = path.dirname(fileURLToPath(import.meta.url));
  const source = path.resolve(cliScriptRunningDir, 'assets', 'design-md', skin, 'DESIGN.md');
  const dest = path.resolve(process.cwd(), 'DESIGN.md');

  if (!(await fs.pathExists(source))) {
    console.error(`  Error: DESIGN.md skin '${skin}' not found. Searched: ${source}`);
    return false;
  }

  await fs.copyFile(source, dest);
  console.log(`  -> Copied to: ${dest}`);
  console.log('  Pair with agents/SKILL.md and your chosen 5H3LL-UI style pack.');
  return true;
}
...
program
  .name('5h3ll-ui')
  .description('Add 5H3LL-UI components and DESIGN.md skins to your project')
  .version(packageVersion);

// Define the 'add' command
program
  .command('add')
  .description('Add one or more 5H3LL-UI components to your project')
...
program
  .command('design')
  .description('Copy a brand DESIGN.md skin to ./DESIGN.md in the current project')
  .argument('[skin]', 'Skin name (cursor, claude, grok, openai, openclaw, hermes)')
  .action(async (skinArg) => {
    try {
      let skin = skinArg;

      if (!skin) {
        const available = await getAvailableDesignSkins();
        if (available.length === 0) {
          console.error('Error: No DESIGN.md skins available in the CLI. Run npm run build first.');
          return;
        }
        ...
        const answers = await inquirer.prompt([
          {
            type: 'list',
            name: 'skin',
            message: 'Which DESIGN.md skin would you like to install?',
            choices: available,
          },
        ]);
        skin = answers.skin;
      ...
      const ok = await addDesignSkin(skin);
      if (!ok) process.exit(1);
    } catch (error) {
      if (error.isTtyError || error.constructor?.name === 'ExitPromptError') {
        console.log('\nOperation cancelled by user.');
      } else {
        console.error('\nAn unexpected error occurred during the design command:', error);
        process.exit(1);
      }
    }
  });
