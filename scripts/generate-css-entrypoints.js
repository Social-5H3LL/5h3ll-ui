...
export async function generateCssEntrypoints({ cssDir = path.resolve('src/css') } = {}) {
  ...
  const baseCss = `@import "./base/base.css";\n@import "./5h3ll_ui-components.css";\n`;

  await writeIfChanged(path.join(cssDir, '5h3ll_ui-components.css'), componentsCss);
  await writeIfChanged(path.join(cssDir, '5h3ll_ui-base.css'), baseCss);
  await writeIfChanged(path.join(cssDir, '5h3ll_ui-base.cdn.css'), '@import "tailwindcss" source(none);\n@import "./5h3ll_ui-base.css";\n');

  for (const style of styles) {
    await writeIfChanged(
      path.join(cssDir, `5h3ll_ui-${style}.css`),
      `@import "./5h3ll_ui-base.css";\n@import "./styles/${style}.css";\n`,
    );
    await writeIfChanged(
      path.join(cssDir, `5h3ll_ui-${style}.cdn.css`),
      `@import "tailwindcss" source(none);\n@import "./5h3ll_ui-${style}.css";\n`,
    );
  }

  await writeIfChanged(path.join(cssDir, '5h3ll_ui.css'), '@import "./5h3ll_ui-vega.css";\n');
  await writeIfChanged(path.join(cssDir, '5h3ll_ui.all.css'), '@import "./5h3ll_ui.css";\n');
  await writeIfChanged(path.join(cssDir, '5h3ll_ui.cdn.css'), '@import "tailwindcss" source(none);\n@import "./5h3ll_ui.css";\n');
