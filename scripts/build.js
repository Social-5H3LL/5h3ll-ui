...
async function build() {
  ...
  const componentsToCombine = ['ui5h3ll.js', 'command.js', 'combobox.js', 'dropdown-menu.js', 'popover.js', 'range.js', 'select.js', 'sidebar.js', 'tabs.js', 'toast.js'];
  ...
  const designMdSourceDir = path.join(projectRoot, 'agents', 'design-md');
  await copyDirRecursive(designMdSourceDir, path.join(cliDistAssetsDir, 'design-md'));
  console.log(`Copied DESIGN.md skins to ${path.join(cliDistAssetsDir, 'design-md')}`);
  ...
  await fs.copyFile(path.join(srcCssDir, '5h3ll_ui.css'), path.join(cssDistDir, '5h3ll_ui.css'));
  await fs.copyFile(path.join(srcCssDir, '5h3ll_ui.all.css'), path.join(cssDistDir, '5h3ll_ui.all.css'));
  await fs.copyFile(path.join(srcCssDir, '5h3ll_ui-base.css'), path.join(cssDistDir, '5h3ll_ui-base.css'));
  await fs.copyFile(path.join(srcCssDir, '5h3ll_ui-base.cdn.css'), path.join(cssDistDir, '5h3ll_ui-base.cdn.css'));
  await fs.copyFile(path.join(srcCssDir, '5h3ll_ui-components.css'), path.join(cssDistDir, '5h3ll_ui-components.css'));
  for (const style of styles) {
    await fs.copyFile(path.join(srcCssDir, `5h3ll_ui-${style}.css`), path.join(cssDistDir, `5h3ll_ui-${style}.css`));
    await fs.copyFile(path.join(srcCssDir, `5h3ll_ui-${style}.cdn.css`), path.join(cssDistDir, `5h3ll_ui-${style}.cdn.css`));
  }
  console.log(`Copied 5H3LL-UI CSS entrypoints to ${cssDistDir}`);

  // Copy split CSS folders used by 5h3ll_ui.css imports.
  ...
  const cdnEntries = ['5h3ll_ui.cdn.css', '5h3ll_ui-base.cdn.css', ...styles.map((style) => `5h3ll_ui-${style}.cdn.css`)];
