  ...
  const initSelect = (selectComponent) => {
    ...
    const handleKeyNavigation = (event) => {
      ...
      if (nextVisibleIndex !== currentVisibleIndex) {
        ...
        scrollOptionIntoListbox(newActiveOption);
      }
    };
    ...
    const openPopover = () => {
      ...
      if (selectedOption) {
        setActiveOption(options.indexOf(selectedOption));
        scrollOptionIntoListbox(selectedOption);
      }
    };
    ...
    selectComponent.dataset.ui5h3llSelectInitialized = true;
    selectComponent.dispatchEvent(new CustomEvent('ui5h3ll:initialized'));
  };

  if (window.ui5h3ll) {
    window.ui5h3ll.register('select', 'div.select:not([data-ui5h3ll-select-initialized])', initSelect);
  }
})();
