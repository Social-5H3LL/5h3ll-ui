(() => {
  const initCommand = (container) => {
    ...
    container.dataset.ui5h3llCommandInitialized = true;
    container.dispatchEvent(new CustomEvent('ui5h3ll:initialized'));
  };

  if (window.ui5h3ll) {
    window.ui5h3ll.register('command', '.command:not([data-ui5h3ll-command-initialized])', initCommand);
  }
})();
