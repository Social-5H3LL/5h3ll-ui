...
description: CSS radio testimonial switcher using shared testimonials data.
icon: users
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Testimonial switcher</strong> layout is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>layout-testimonial-switcher</code>.</p>
</div>
...
{% set code %}
{% include "partials/layouts/testimonial-switcher.njk" %}
{% endset %}
{{ code_preview("layout-testimonial-switcher", code, "w-full") }}
