...
title: Features grid
description: Eight feature cards in a responsive grid with Lucide icons.
icon: layout-grid
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Features grid</strong> layout is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>layout-features-grid</code>.</p>
</div>
...
{% set code %}
{% include "partials/layouts/features-grid.njk" %}
{% endset %}
{{ code_preview("layout-features-grid", code, "w-full") }}
