...
description: Split testimonial showcase with static featured quote.
icon: quote
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Testimonial showcase</strong> layout is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>layout-testimonial-showcase</code>.</p>
</div>
...
{% set code %}
{% include "partials/layouts/testimonial-showcase.njk" %}
{% endset %}
{{ code_preview("layout-testimonial-showcase", code, "w-full") }}
