  ...
  items=[
    {
      ...
      label: "Theme",
      items: [
        { type: "item", value: "light", label: "Light" },
...
<script src="https://cdn.jsdelivr.net/npm/@social-5h3ll/5h3ll-ui@{{ pkg.version }}/dist/js/select.min.js" defer></script>{% endset %}
...
<div class="flex flex-wrap gap-2 my-6">
  ...
  <a class="badge-outline" href="https://github.com/social-5h3ll/5h3ll-ui/blob/main/src/js/select.js" target="_blank">
    select.js
    {% lucide "arrow-right" %}
  </a>
</div>
...
<section class="prose">
  <dl>
    ...
    <dt><code>ui5h3ll:popover</code></dt>
    <dd>When the popover opens, the component dispatches a custom (non-bubbling) <code>ui5h3ll:popover</code> event on <code>document</code>. Other popover components (Combobox, Dropdown Menu, Popover and Select) listen for this to close any open popovers.</dd>
...
{% set code_value %}<script>
  ...
  selectComponent.addEventListener('ui5h3ll:initialized', () => {
    // Get value
...
<div class="flex flex-wrap gap-2 my-6">
  ...
  <a class="badge-outline" href="https://github.com/social-5h3ll/5h3ll-ui/blob/main/src/jinja/select.html.jinja" target="_blank">
    Jinja macro
    {% lucide "arrow-right" %}
  </a>
  <a class="badge-outline" href="https://github.com/social-5h3ll/5h3ll-ui/blob/main/src/nunjucks/select.njk" target="_blank">
    Nunjucks macro
    {% lucide "arrow-right" %}
  </a>
</div>
...
<section class="prose">
  <p>Use <code>data-align</code> on the popover to align the list to the trigger edge. 5H3LL-UI keeps the popup below the control; it does not implement shadcn/ui's selected-item-over-trigger positioning.</p>
</section>

{% set code %}
<div class="flex flex-wrap gap-3">
  {{ select(
    id="select-align-start",
    trigger_attrs={"class": "w-[180px]"},
...
{% endset %}
{{ code_preview("select-align", code | prettyHtml) }}
...
{% endcall %}
{% endset %}
{{ code_preview("select-groups", code | prettyHtml) }}
...
<div role="group" class="field" data-invalid="true">
  ...
  {{ select(
    id="select-invalid",
    trigger_attrs={"aria-invalid": "true", "class": "w-[220px]"},
    items=[
  ...
  <p class="field-error" role="alert">Select a role to continue.</p>
</div>
{% endset %}

{{ code_preview("select-invalid", code | prettyHtml, "w-full max-w-sm") }}
...
<div role="option" data-value="pie">
  <span class="flex items-center gap-2">
...
<section class="prose">
  <p>Enable multiple selection by adding <code>aria-multiselectable="true"</code> to the <code>listbox</code>. The selected options are displayed as a comma-separated list in the trigger button. Use <code>data-label</code> to specify clean text labels for options that contain HTML (like icons), otherwise the text content will be extracted automatically.</p>
</section>
...
<section class="prose">
  <p>Set <code>dir="rtl"</code> on the select root or a parent element.</p>
</section>
...
{% endset %}
{{ code_preview("select-rtl", code | prettyHtml) }}
