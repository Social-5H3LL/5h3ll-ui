...
description: Split contact layout with decorative map and action cards.
icon: map-pin
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Contact simple</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-contact-simple</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/contact-simple.njk" %}
{% endset %}
{{ code_preview("block-contact-simple", code, "w-full") }}
