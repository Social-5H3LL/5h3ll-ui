...
title: CTA learning split
description: Split CTA card with learner avatars and hero image.
icon: graduation-cap
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>CTA learning split</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-cta-learning-split</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/cta-learning-split.njk" %}
{% endset %}
{{ code_preview("block-cta-learning-split", code, "w-full") }}
