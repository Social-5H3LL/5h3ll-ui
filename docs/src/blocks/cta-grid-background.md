...
title: CTA grid background
description: Centered CTA over masked grid background pattern.
icon: grid-3x3
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>CTA grid background</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-cta-grid-background</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/cta-grid-background.njk" %}
{% endset %}
{{ code_preview("block-cta-grid-background", code, "w-full") }}
