...
description: Floating avatar newsletter hero with inline form.
icon: send
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Newsletter split</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-newsletter-split</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/newsletter-split.njk" %}
{% endset %}
{{ code_preview("block-newsletter-split", code, "w-full") }}
