...
title: Contact card
description: Premium contact form with benefit list and info cards.
icon: contact
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Contact card</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-contact-card</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/contact-card.njk" %}
{% endset %}
{{ code_preview("block-contact-card", code, "w-full") }}
