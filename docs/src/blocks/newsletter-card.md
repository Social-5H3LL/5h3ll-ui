...
description: Split newsletter card with benefits list.
icon: mail-plus
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Newsletter card</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-newsletter-card</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/newsletter-card.njk" %}
{% endset %}
{{ code_preview("block-newsletter-card", code, "w-full") }}
