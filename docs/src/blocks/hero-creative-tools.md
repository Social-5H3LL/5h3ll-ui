...
title: Hero creative tools
description: Centered hero with announcement pill and primary actions.
icon: wand-sparkles
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Hero creative tools</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-hero-creative-tools</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/hero-creative-tools.njk" %}
{% endset %}
{{ code_preview("block-hero-creative-tools", code, "w-full") }}
