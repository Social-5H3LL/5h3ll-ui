...
description: Centered hero with avatar stack, star rating, and dual CTAs.
icon: sparkles
toc:
  - label: Overview
    id: overview
...
<div class="prose">
  <p>The <strong>Hero social proof</strong> block is composed from 5H3LL-UI semantic tokens and existing components. Hook class: <code>block-hero-social-proof</code>.</p>
</div>
...
{% set code %}
{% include "partials/blocks/hero-social-proof.njk" %}
{% endset %}
{{ code_preview("block-hero-social-proof", code, "w-full") }}
